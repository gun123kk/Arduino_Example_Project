...
void loop() { 
  //controll.run();  
  JogMode();     
  int receiveStepI2C[0];
  int counter=0;  
  Wire.requestFrom(2,16);
  while (Wire.available()) { 
    receiveStepI2C[counter] = Wire.read();
    counter++;  
  }
  receiveStep=receiveStepI2C[0]*256+receiveStepI2C[1];
   Serial.println(receiveStep);  
  if(receiveStep>=1001){   
    moreThan1000=1;
  }  
  LCD_Callback();   
...
void   thread_initial(){  
  ...
  controll.add(&LCD_Thread);  // Adds myThread to the controll
 }
//====================================
//JOG模式
//====================================
...
ISR (TIMER5_OVF_vect) {
  TIMSK5 = 0x00;     //timer5 stop
  TCNT5 = Timer5CountSet[timer5set];  //2 =100uS
 Timer5Counter++;
  //jog mode
